...
# not used    parameter_list["MATRIX_version"] = self.plot_properties.get("version","0.0.1alpha") # set it to something meaningful later
        if self.plot_properties.get("legend","right") == "right":
            ...
            parameter_list["key_x"]  = 0.40 # x-position of key 
            ...
            parameter_list["key_x"]  = 0.1 # x-position of key 
...
#    all_plots = glob.iglob(pjoin(os.getcwd(),"MATRIX_NNLO_31_inclusive_sumETscale-run/distributions/*.dat"))
    all_plots = glob.iglob(pjoin(os.getcwd(),"datfiles/exa-Ht2-MSbarQ-run/distributions/*.dat"))
...
#        plot_MATRIX_NNLO_31_inclusive_sumET = plot
        plot_exact = plot
        plot_mass = plot.replace("exa","mass")
        plot_soft = plot.replace("exa","soft")
        plot_comb = plot.replace("exa","comb")
        plot_combsep2 = plot.replace("exa","combsep2")
#        plot_novirt = plot.replace("exact","zero")
...
#        gnu.add_curve(plot_minnlopt,{"format" : "histogram", "label" : "MiNNLO K_Q=0.25 (LHE)","line_style" : 1})
        gnu.add_curve(plot_exact,{"format" : "histogram", "label" : "M1M0_Q (exact)","line_style" : 1})
        gnu.add_curve(plot_mass,{"format" : "histogram", "label" : "M1M0_Q (MA)","line_style" : 3})
        gnu.add_curve(plot_soft,{"format" : "histogram", "label" : "M1M0_Q (SA)","line_style" : 4})
        gnu.add_curve(plot_comb,{"format" : "histogram", "label" : "M1M0_Q (CAold)","line_style" : 2})
        gnu.add_curve(plot_combsep2,{"format" : "histogram", "label" : "M1M0_Q (CAnew)","line_style" : 5})
#        gnu.add_curve(plot_novirt,{"format" : "histogram", "label" : "NLO_{LHE} (0A)","line_style" : 4})
...
#------------------------------------------------------------------------------------
        if gnu.get_name().startswith("detaHiggstop"):
            ...
            gnu.set_plot_properties("yunit","[fb]")
            gnu.set_plot_properties("legend","down left")
