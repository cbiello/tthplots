...
#    all_plots = glob.iglob(pjoin(os.getcwd(),"MATRIX_NNLO_31_inclusive_sumETscale-run/distributions/*.dat"))
    all_plots = glob.iglob(pjoin(os.getcwd(),"datfiles/nov8-collectDAT/SA-RFvar-run/distributions/*.dat"))
...
#        gnu.add_curve(plot_exact,{"format" : "histogram", "label" : "NLO_{PS} (exact)","line_style" : 1})
        gnu.add_curve(plot_scales,{"format" : "histogram", "label" : "MiNNLO_{PS} (SA, RF)","line_style" : 2})
        gnu.add_curve(plot_eta,{"format" : "histogram", "label" : "MiNNLO_{PS} (SA, ET)","line_style" : 1})
