...
#{{{ def: check_curve_properties(self,properties)
    def check_curve_properties(self,properties):
        ...
        allowed_properties["onlyuncertainties"]  = [False,True] # if empty everything is allowed (specify later)
...
#{{{ def: check_plot_properties(self,properties,value = None)
    def check_plot_properties(self,properties,value = None):
        ...
        allowed_properties["normalization2"] = [x for x in range(1,len(self.curve_list)+1)]
...
set style line 11 dt (3,3) lc rgb \"dark-blue\" lw 0.05
set style line 12 dt (3,3) lc rgb \"dark-blue\" lw 0.05
#MiNLOp 5FS (LHE 3, PS 4)
set style line 3 dt (9,6) lc rgb \"orange\" lw 1
set style line 4 dt (3,3,3,3) lc rgb \"black\" lw 1
set style line 13 dt (8,4,8,4,3,4) lc rgb \"orange\" lw 0.05
set style line 14 dt (3,3,3,3) lc rgb \"black\" lw 0.05
#MiNNLOPS 4FS (LHE 7 PS 8)
set style line 7 dt (3,3) lc rgb \"light-red\" lw 1 
set style line 8 dt (3,3) lc rgb \"dark-orange\" lw 1
set style line 17 dt (10,3,3,3) lc rgb \"light-red\" lw 0.05
set style line 18 dt (10,3,3,3) lc rgb \"dark-orange\" lw 0.05
#MiNLOp 4FS (LHE 9, PS 3 but activate it)
set style line 9 lc rgb \"web-green\" lw 1
#set style line 3 lc rgb \"dark-green\" lw 1
# set style line 19 dt (8,4,8,4,3,4) lc rgb \"web-green\" lw 0.05
# set style line 13 dt (8,4,8,4,3,4) lc rgb \"dark-green\" lw 0.05
#NLOPS 4FS (LHE 21, PS 22 but deactivate the others)
#set style line 1 lc rgb \"orange\" lw 1
#set style line 2 lc rgb \"coral\" lw 1
#set style line 11 dt (3,3) lc rgb \"orange\" lw 0.05
#set style line 12 dt (3,3) lc rgb \"coral\" lw 0.05


set style line 1 lc rgb \"dark-blue\" lw 1
set style line 11 lc rgb \"dark-blue\" lw 0.2 dt solid
set style line 2 lc rgb \"slateblue1\" lw 1                                    
set style line 12 dt (10,3,3,3) lc rgb \"slateblue1\" lw 0.05


#MiNNLOPS 5FS (LHE 1, PS 2)
set style line 5 lc rgb "#501450" lw 1
set style line 6 lc rgb \"dark-violet\" lw 1
set style line 15 lc rgb "#501450" lw 0.2
set style line 16 dt (3,3) lc rgb \"dark-violet\" lw 0.05
    ...
    def get_gnuplot_plot_main_frame(self):
        ...
        for curve in self.curve_list:
            ...
            if prop[curve].get("format") == "data":
                ...
                if prop[curve].get("onlyuncertainties",True):
                    print("onlyunc")
                else:
            ...
            if prop[curve].get("uncertainties",True) and prop[curve].get("show_uncertainties",True):
                if prop[curve].get("onlyuncertainties",True):
                    plot_main_frame += "\"%s\" using 1:4:6 with filledcurves ls %s fs transparent solid 0.35 title \"%s\"" % (curve,line_style,prop[curve].get("label",order[counter-1]))
                else:
...
set label \"%(category)s\" at graph 0.05, 1.39
...
#
            if prop[curve].get("uncertainties",True) and prop[curve].get("show_uncertainties",True):
                if prop[curve].get("onlyuncertainties",True):
                    plot_ratio += "\"<paste %s %s\" using 1:($4/$%s):($6/$%s) with filledcurves ls %s fs transparent solid 0.35 %s" % (curve,norm,2+columns,2+columns,line_style,"title \"%s\"" % prop[curve].get("label",order[counter-1]) if prop[curve].get("exclude_from_main" ,False) or counter in self.plot_properties.get("exclude_from_main",[]) else "notitle\\\n")
                else:
...
#        plot_exact = plot
        ...
        plot_NLOcomb = plot.replace("nov24-HaafromPY8-scales","nov26-HaafromPY8-NLOcomb")
        plot_NLOcomberr = plot.replace("nov24-HaafromPY8-scales","nov26-HaafromPY8-NLOcomberr")
...
#        gnu.add_curve(plot_exact,{"format" : "histogram", "label" : "NLO_{PS} (exact)","line_style" : 1})
        gnu.add_curve(plot_NLO,{"format" : "histogram", "label" : "NLO+PS (exact)","line_style" : 3, "onlyuncertainties" : False})
        gnu.add_curve(plot_scales,{"format" : "histogram", "label" : "MiNNLO_{PS} (CA)","line_style" : 2, "exclude_from_ratio2" : True, "onlyuncertainties" : False})
        gnu.add_curve(plot_eta,{"format" : "histogram", "label" : "MiNNLO_{PS} (CA, 2l-unc)","line_style" : 1, "exclude_from_ratio2" : True})
        gnu.add_curve(plot_NLOcomb,{"format" : "histogram", "label" : "NLO+PS (CA)","line_style" : 6, "exclude_from_main" : True, "exclude_from_ratio" : True, "onlyuncertainties" : False})
        gnu.add_curve(plot_NLOcomberr,{"format" : "histogram", "label" : "NLO+PS (CA, 1l-unc)","line_style" : 5, "exclude_from_main" : True, "exclude_from_ratio" : True})
...
#            gnu.add_curve(plot_MATRIX_NNLO_30,{"format" : "histogram", "label" : "MATRIX 30","line_style" : 4})
        ...
        gnu.set_plot_properties("normalization2",1)
...
#            gnu.set_plot_properties("rebin",1)
        ...
        if "aafidR2" in gnu.get_name():
            gnu.set_plot_properties("category","fidudial cuts {/Symbol gg}")
        ...
        if gnu.get_name().startswith("PTH"):
            ...
            gnu.set_plot_properties("xunit","GeV")
            gnu.set_plot_properties("xmin",0)
            gnu.set_plot_properties("ymax",0.01)
